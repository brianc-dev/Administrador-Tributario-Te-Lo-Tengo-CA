    ...
    private fun createCompanyTable() {
        database.connectDatabase().use { con ->
            ...
            con.createStatement().use {stm ->
                stm.execute("CREATE TABLE IF NOT EXISTS `company`(`rif` VARCHAR(10)  PRIMARY KEY NOT NULL, `name` VARCHAR(50) NOT NULL, `address` VARCHAR(100), `telephone` VARCHAR(15), `telephone` VARCHAR(15), `email` VARCHAR(254), `alias` VARCHAR(50) NOT NULL)")
            }
